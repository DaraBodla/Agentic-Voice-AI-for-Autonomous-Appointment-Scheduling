...
class CampaignEngine {
  static Future<List<Provider>> lookupProviders({
    ...
    if (EnvConfig.hasGooglePlaces) {
      try {
        providers = await GooglePlacesService.searchProviders(
    ...
    if (providers.isEmpty) {
      providers = DemoData.getProvidersByType(serviceType);
      if (providers.isEmpty) {
        providers = DemoData.providers.take(maxResults).toList();
      }
    }
    return providers.take(maxResults).toList();
  }
